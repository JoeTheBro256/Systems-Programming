...
int transfer_cards(struct player* src, struct player* dest, char rank){
	...
	int count = 0;
	while(iterator != NULL){
		if((*iterator).top.rank[0] == rank){//loop till the src has a particular card
			//add_card(dest,&(iterator->top));//add card to dest
			remove_card(src,&(iterator->top));//remove card from src
			add_card(dest,&(iterator1->top)); //Slight edge-case found by running remove before add
			count++;
		}
		iterator = iterator->next;//next card/hand
	}
	return count;
}
